        ...
        >
            ...
            <div className={view === "list" ? "flex-1 flex flex-col gap-2" : "mt-3"}>
                ...
                {accessory.rating !== undefined && (
                    <div className="flex items-center gap-1 mt-1">
                        {[...Array(5)].map((_, i) => (
                            <FaStar
                                key={i}
                                className={`transition-colors duration-300 ${i < Math.round(accessory.rating ?? 0)
                                    ? "text-yellow-400"
                                    : "text-gray-300 group-hover:text-yellow-200"
                                    }`}
                            />
                        ))}
                        <span className="ml-2 text-sm text-gray-500">{accessory.rating}</span>
                    </div>
                )}
                ...
                {accessory.stock !== undefined && (
                    <span
                        className={`inline-block mt-1 text-xs px-3 py-1 rounded-full font-medium ${accessory.stock > 0
